...
export async function fetchTeam() {
  try {
    ...
    engTeamMembers.forEach((member) => {
      ...
      figure.innerHTML = `<img fetchpriority="high" src="https://picsum.photos/200" alt="profile picture"/>
                          <figcaption>
                            <h3 style="color: black;">${member.name}</h3>
                            <p style="color: black;">username: ${member.username}</p>
                            <p style="color: black;">email: ${member.email}</p>
                        </figcaption>
    ...
    markTeamMembers.forEach((member) => {
      ...
      figure.innerHTML = `<img fetchpriority="high" src="https://picsum.photos/200"/>
                          <figcaption>
                            <h3>${member.name}</h3>
